...
<script type="text/html" data-template-name="link out">
    ...
    <div class="form-row">
        <input type="checkbox" id="node-input-onlyConnected" style="display: inline-block; width: auto; vertical-align: top;">
        <label style="width: auto;" for="node-input-onlyConnected"><span data-i18n="link.showConnectedOnly"></span></label>
    </div>
</script>
...
(function() {
    ...
    function onEditPrepare(node,targetType) {
        ...
        const isConnectedFilter = function (item) {
            if (node.onlyConnected && !item.selected) {
                return false;
            }
            return true;
        }

        treeList.treeList("filter", isConnectedFilter);
        ...
        $("#node-input-onlyConnected").change( function() {
            node.onlyConnected = $(this).is(':checked');
            search.searchBox("change");
        });
        ...
        const flowsFiltered = flows.filter(function(f) { 
            ...
            let rex = /^link (?:in|out) ([0-9]+)$/i;
            f.children.sort(function(a,b) {
                // sort the "link in|out ##" nodes by number
                let ar = a.label?.match(rex)?.[1];
                let br = b.label?.match(rex)?.[1];
                if (ar && br) {
                    a = parseInt(ar);
                    b = parseInt(br);
                } else {
                ...
                // Would it be better to use .localCompare() here?
        ...
        flowsFiltered.forEach((f) => {
            f.children.forEach((c) => {
                if (c.disabled) {
                    let item = treeList.treeList('get', c.id);
                    let label = item?.treeList.label.find(".red-ui-treeList-label-text");
                    label?.addClass("red-ui-link-in-disabled");
                }
            });
        });
    ...
    function resizeNodeList() {
        ...
        var height = $(".red-ui-tray-content").height();
